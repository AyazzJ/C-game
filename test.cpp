...
void handleSubGameClick(SubGame* sub, int x, int y);
void aiMove(SubGame* sub);
...
void aiRandomMove(SubGame* sub);
void aiMediumMove(SubGame* sub);
int tryWinningMove(SubGame* sub, char symbol);
void aiHardMove(SubGame* sub);
int minimax(SubGame* sub, int depth, int isMaximizing);
...
// AI makes a medium difficulty move
void aiMediumMove(SubGame* sub) {
    if (!tryWinningMove(sub, 'O')) {
        if (!tryWinningMove(sub, 'X')) {
            aiRandomMove(sub);
        }
    }
}

// Try to make a winning move for the given symbol
int tryWinningMove(SubGame* sub, char symbol) {
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            if (sub->board[i][j] == ' ') {
                sub->board[i][j] = symbol;
                if (checkSubWin(sub->board)) {
                    return 1;
                }
                sub->board[i][j] = ' ';
...
// AI makes a hard difficulty move using minimax
void aiHardMove(SubGame* sub) {
    ...
    if (isMaximizing) {
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                if (sub->board[i][j] == ' ') {
                    sub->board[i][j] = 'O';
                    int score = minimax(sub, depth + 1, 0);
                    sub->board[i][j] = ' ';
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                if (sub->board[i][j] == ' ') {
                    sub->board[i][j] = 'X';
                    int score = minimax(sub, depth + 1, 1);
                    sub->board[i][j] = ' ';
...
int main(int argc, char* argv[]) {
    ...
    // Difficulty selection if PvE is chosen
    if (gameMode == 1) {
